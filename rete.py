...
def rete(input_shape=None, weight_decay=0., batch_shape=None, classes=5):
    ...
    x = Dropout(0.5)(x)
    x = Conv2D(classes, (3, 3),kernel_initializer='he_normal', activation='linear', padding='same', kernel_regularizer=l2(weight_decay))(x)
    
    x = tf.keras.layers.UpSampling2D(16,interpolation='bilinear')(x)

    #x = Conv2D(classes, 1,strides=(1, 1), activation='softmax', padding='valid')(x)
