...
def rete(input_shape=None, weight_decay=0., batch_shape=None, classes=5):
    ...
    # Block 5
    x = Conv2D(1024, (3, 3), activation='relu', padding='same',dilation_rate=(8,8), name='fc3', kernel_regularizer=l2(weight_decay))(x)
    x = Dropout(0.5)(x)
    x = Conv2D(1024, (3, 3), activation='relu', padding='same', name='fc4', kernel_regularizer=l2(weight_decay))(x)
    x = Dropout(0.5)(x)
    x = Conv2D(classes, (3, 3),  kernel_initializer='normal',dilation_rate=(4,4), activation='linear', padding='same', strides=(1, 1), kernel_regularizer=l2(weight_decay))(x)
