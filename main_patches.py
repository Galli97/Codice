...
model.compile(optimizer = optimizer, loss = loss_fn , metrics =[iou_coef(y_true,y_pred), dice_coef()],sample_weight_mode='temporal')#tf.keras.metrics.SparseCategoricalAccuracy()#MyMeanIoU(num_classes=5)#loss_weights=loss_weights#[tf.keras.metrics.SparseCategoricalAccuracy()]#[tf.keras.metrics.MeanIoU(num_classes=5)])#['accuracy'])#[sparse_accuracy_ignoring_last_label])#,sample_weight_mode='temporal')
