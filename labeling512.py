...
# ####### PERCORSO IN LOCALE #########
path = r"C:\Users\Mattia\Documenti\Github\Codice\cropped_images512.npy"
path1 =  r"C:\Users\Mattia\Documenti\Github\Codice\cropped_labels512.npy"
...
num_classes=5
tmp1 = np.empty((len(crop_images_list), 512, 512, 3), dtype=np.uint8)  #Qui ho N immagini
tmp2 = np.empty((N, 512, 512, 1), dtype=np.uint8)  #Qui ho N labels, che portano l'informazione per ogni pixel. Nel caso sparse avrò un intero ad indicare la classe
tmp3 = np.empty((N, 512, 512, 3), dtype=np.uint8)  #Qui ho N immagini
...
for t in range (0,len(crop_labels_list)):
    ...
    if(count==N):
        ...
        for m in range(0,512):
            ...
            for j in range(0,512): 
                ...
                if(flag_bigrock==True and flag_soil==True):
                    ...
                    counter_null+=1;
                    if (counter_bigrock>portion):
                        flag_null=True;
                elif all(channels_xy==bedrock):      #BEDROCK
                    counter_bedrock+=1;
                    if (counter_bedrock>portion):
                        flag_bedrock=True
                elif all(channels_xy==sand):    #SAND
                    counter_sand+=1;
                    if (counter_sand>portion):
                        flag_sand=True
        ...
        if(counter_soil>portion and counter_soil_reduce>portion*2):
            flag_soil=False      
        ...
        if (flag_bigrock==True and flag_sand==True):
            ...
            # new_label = np.empty((512, 512, 1), dtype=np.uint8)  #inizializzo una nuova lista che andrà a contenere le informazioni per ogni pixel
